...
**В workspace:** `cl-platform`, `cl-ipc`, `cl-process`, `cl-testshell`, `cl-net`, `cl-dom`, `cl-html`, `cl-style`, `cl-fonts`, `cl-layout`, `cl-paint`, `cl-gfx`, `chromelight`. Fuzz-таргеты (6): `ipc_decode` (M0), `url_parse`, `html_parse`, `css_stylesheet`, `display_list_validate`, `display_list_rasterize` (M1a; последний добавлен финальным ревью — `cl_gfx::cpu::rasterize` за валидатором).

**Конформанс (гейты — измерения, не цели):** WPT `urltestdata.json` 828/893 (92.7%), гейт 0.92 = потолок crate `url` 2.5.8 (Servo фиксирует те же провалы). html5lib tree-construction 1307/1313 (99.5%), 0 паник, гейт 0.90; 6 провалов — пробелы html5ever 0.39 (см. `docs/SPEC_REGISTRY.md`). **Знаменатель — 36 вендоренных `.dat`-файлов** из списка Task 9, а не весь апстрим-корпус: `svg.dat`, `math.dat`, `plain-text-unsafe.dat`, `scriptdata01.dat`, `foreign-fragment.dat` и др. не вендорены, так что SVG/MathML-ветки дерева построения конформансом не покрыты вообще. Reftests: **22/22 PASS** (`cargo run -p cl-testshell -- reftest`, `crates/testshell/tests/reftests.rs`; 20 из брифа плюс `margin-collapse-siblings-whitespace`, регрессия на баг ниже, и `max-width-auto-margins`, регрессия на B1 финального ревью). Golden dumps — 5 стадий (dom/style/box-tree/fragments/display-list). Смоук: 1 МБ страница 1.51 с (release) / ~4.7 с (debug), peak RSS ~330 МиБ — **над** критерием «< 300 МБ» (`docs/history/perf-m1a-2026-09.md`).
...
1. **Квадратичный парсинг/стилизация на глубокой вложенности** — потенциальный DoS-вектор на враждебной странице с искусственно большой глубиной дерева; не пофикшено в M1a, приоритет для M2 перед тем, как открывать произвольный веб. DOM-половина **закрыта финальным ревью**: `Document::check_attach` больше не ходит по всей цепочке предков, когда вставляемый узел бездетный (а у html5ever он всегда такой) — цепочка 50 000 в глубину строится за 15 мс вместо 8,6 с; `parse_document_str` на глубине 8 000 (debug, медиана из 5) 2,529 с → 2,376 с. Остаток (~2,4 с) — внутри html5ever и `cl-style`, не трогали. **При перезамере стилизации мерить в RELEASE:** под `debug_assertions` stylo гоняет `StyleBloom::assert_complete`, который сам O(depth) на элемент, т.е. debug-профиль показывает квадратичность, которой в release может не быть. **Ещё один O(width²)-риск** (пока не наблюдаемый: M1a не реализует `:nth-*`/`+`/`~`): `prev_sibling_element`/`next_sibling_element` в `cl-style::handle` линейно сканируют по списку сиблингов, пропуская не-элементы, поэтому селектор, который спрашивает соседа у каждого из n сиблингов, даёт O(n²).
...
8. ~~**`FontNotBundled` не протестирован**~~ — **закрыто финальным ревью**. Заодно поправлена атрибуция: `FontNotBundled` — вариант `cl_gfx::GfxError` (растеризация получила `FontKey`, которого нет в её `FontDb`; с M1b такой ключ приезжает по IPC от рендерера со старой базой шрифтов), а не ошибка `cl-fonts` про нераспознанное семейство — `cl_fonts::FontDb::key_for` на неизвестном семействе просто возвращает `None`. Тест: `rasterize_should_report_font_not_bundled` (`crates/gfx/tests/cpu.rs`), через новый doc-hidden `FontKey::from_raw` за feature-флагом `arbitrary`.
...
10. **Профилирование памяти/перфа `cl-layout` (два известных подозреваемых)** — оба найдены финальным ревью, оба вне мандата этой волны: (а) `span_item` в `crates/layout/src/text.rs:496` линейно сканирует список спанов для *каждого* кластера, т.е. O(символов × спанов) на абзац с большим числом инлайнов; (б) `LayoutStyle` (с `Vec<String>` `font_family` внутри) клонируется по разу на каждый фрагмент, на каждую `Line` и на каждый `Text` — прямой вклад в peak RSS из долга №2.
11. **17+ мелких находок ревью (deferred)** — живут в SDD ledger как строки `minor (deferred)`; триаж — на финальном ревью ветки, не в этом списке поимённо.
...
- **Финальное ревью ветки (3 ревьюера) + волна правок, 4 коммита.** Два Critical: (B1) §10.4-клэмп min/max никогда не применялся к `width:auto` — `max-width` + `margin:0 auto` не центрировал и не ограничивал; (C1) отсечение офскрин-айтемов в `cl-paint` и проверка границ в `validate` требовали полного *вхождения* прямоугольника в инфлейтнутый вьюпорт, из-за чего фон контейнера выше `viewport + 4096px` (обычный `height:5000px`) выбрасывался целиком и страница рисовалась пустой — оба теста стали *пересечением*. Important: `min-height` не блокировал схлопывание нижнего margin-а; `box-sizing` не применялся к `min-*`/`max-*`; `overflow != visible` теперь создаёт BFC и рвёт цепочку схлопывания (CSS 2.1 §9.4.1); `ParseOutput::parse_errors` ограничен 256 + счётчик подавленных; `h1`–`h6 { font-weight: bold }` в UA-листе; `check_attach` больше не квадратичен на глубокой вложенности; шестой fuzz-таргет `display_list_rasterize` (774 563 прогона за 60 с, 0 падений). Ни один из 15 golden-снапшотов не изменился. Реф-пар стало 22 (`max-width-auto-margins`). Детали: `.superpowers/sdd/2026-09-07-m1a-static-pipeline/final-fix-wave-report.md`.
