...
| Layout | block, inline, positioned, floats | M1a: partial (block + inline + `position:relative` реализованы и покрыты 21/21 reftest-парами; floats, absolute/fixed — нет) / M1 |
...
| Test infra | `cl-testshell`: render/dump/compare/reftest CLI, 21 reftest-пара (`crates/testshell/tests/ref`), golden dumps 5 стадий, 5 fuzz-таргетов | M1a: done |
