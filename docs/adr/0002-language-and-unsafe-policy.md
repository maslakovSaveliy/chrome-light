...
- `#![forbid(unsafe_code)]` во всех crate-ах, кроме: `cl-platform`, `cl-process`, `cl-gfx` (backend модули), `cl-js` (V8 FFI), `cl-bindings/runtime`. Там — `deny` на crate + `allow` на модуль, `unsafe_op_in_unsafe_fn = deny`, `undocumented_unsafe_blocks = deny`.
