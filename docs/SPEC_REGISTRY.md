...
| css-text-3 §4.1.1, §5 (line breaking) | white-space processing, line breaking, `text-align` | `cl-layout::whitespace`, `::text` (parley), `::inline` | `crates/layout/src/whitespace.rs` (unit), `crates/layout/tests/inline.rs`, `tests/fragment_tree_goldens.rs`; wpt/css/css-text не подключён | partial | M1a: `white-space` только `normal`/`pre` (остальные значения складываются в них в `style_adapt`); нет `pre-wrap`/`pre-line`/`break-spaces`, `text-transform`, `letter-spacing`/`word-spacing`, `text-indent`, bidi (только LTR), вертикальных writing modes. Перенос по словарю для CJK/тайского (`parley/complex-scripts`) выключен. Высота строкового бокса = `line-height` контейнера: собственные `line-height` вложенных инлайнов и `vertical-align` не реализованы. Схлопывающийся пробел в конце строки «висит» (глифы не выдаются, в ширину строки не входит). Инлайновые рамки/фон/padding не рисуются. |
