...
| css-display-3, CSS2 §9–10 | block/inline formatting | `cl-layout::block`, `::inline` | `crates/layout/tests/block.rs`, `crates/layout/tests/inline.rs`, `crates/layout/tests/box_tree_goldens.rs`, `crates/layout/tests/fragment_tree_goldens.rs`, `crates/testshell/tests/reftests.rs` (20 reftest-пар); wpt/css/CSS2, css-display не подключены | partial | M1a: block-in-inline блокифицируется (инлайновый контейнер становится блочным боксом) вместо сплита по CSS 2.1 §9.2.1.1 (тест `box_tree_should_blockify_inline_with_block_child`); self-collapsing пустые блоки (CSS 2.1 §8.3.1 — блок с `height:auto`, без padding/border/контента схлопывающий свой top/bottom margin в один) не реализованы (`crates/layout/src/block.rs`, "What is still not implemented") — обнаружено на Task 23 при написании reftest-пар с пробельным текстовым узлом между соседними `<div>`: он превращается в реальный (нулевой высоты) anonymous block, который вместо коллапса до max() суммирует соседние margin-ы; страницы reftest-набора написаны без такого пробела, чтобы не задевать это ограничение (задокументировано в MEMORY.md "Долги"), полноценная поддержка — M2. |
