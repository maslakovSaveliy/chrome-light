//! Shared setup for `cl-paint`'s integration tests: parses and styles a document, lays it
...
//!
//! Copied rather than reused: `cl-paint`'s tests must not depend on `cl-layout`'s test files
//! (Task 19's controller ruling), so this is a second, independent copy of the same
...
pub const BASE_URL: &str = "file:///paint/test.html";
    ...
    let warnings = engine
        .collect_document_sheets(&doc, &|_url: &Url| -> Result<Vec<u8>, NetError> {
            Err(NetError::UnsupportedScheme(
                "paint test fixtures load no external sheets".into(),
            ))
        })
        .expect("collect sheets");
    assert!(
        warnings.is_empty(),
        "paint test fixtures must not depend on external stylesheets: {warnings:?}"
    );
